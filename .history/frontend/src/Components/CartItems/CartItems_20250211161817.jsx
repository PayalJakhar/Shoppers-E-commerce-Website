...
const CartItems = () => {
    ...
    const totalAmount = donationCount >= 5 ? (getTotalCartAmount() * 0.7).toFixed(2) : getTotalCartAmount();
    ...
    //             method: "POST",
    //             headers: { "Content-Type": "application/json" },
    //             body: JSON.stringify({ amount: totalAmount, email: userEmail }) // Send email with order
    //         });
    
    //         const data = await res.json();
    //         if (!data || !data.order) throw new Error("Payment order failed!");
    
    //         handlePaymentVerify(data.order, userEmail); // Pass email to next function
    //     } catch (error) {
    //         console.error("Payment Error:", error);
    //         toast.error("Payment initialization failed!");
    //     }
    ...
    // const handlePaymentVerify = async (data, userEmail) => {
    //     if (!window.Razorpay) {
    //         toast.error("Razorpay SDK not loaded!");
    //         return;
    //     }
    ...
    //         handler: async (response) => {
    //             try {
    //                 response.email = userEmail; // Attach email to response object
    
    //                 const res = await fetch(`${process.env.REACT_APP_BACKEND_HOST_URL}/api/payment/verify`, {
    //                     method: "POST",
    //                     headers: { "Content-Type": "application/json" },
    //                     body: JSON.stringify(response) // Send email with verification
    //                 });
                ...
                <div className="cartitems-total">
                    ...
                    <div>
                        ...
                        <div className="cartitems-total-item">
                            ...
                            <p style={{color:'green'}}>{donationCount >= 5 ? '30%' : '0%'}</p>
                        </div>
                        <hr />
                        <div className="cartitems-total-item">
