...
const Donation = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    pincode: "",
    donationDate: "",
    ngo: "", // Ensure NGO is included
  });
  
  const handleNGOSelect = (ngo) => {
    setSelectedNGOs(ngo);
    setFormData((prev) => ({ ...prev, ngo: ngo.name }));
  };
  ...
  const handleDonationSubmit = async (e) => {
    ...
    const formattedClothesDetails = clothesDetails.map((item) => ({
      type: item.type || "Unknown",
      quantity: parseInt(item.quantity) || 0, // Ensure numeric values
      damage: parseFloat(item.damage) || 0, // Ensure numeric values
  ...
  const handleDonationSubmit = async (e) => {
    ...
    if (!donationData.name || !donationData.email || !donationData.phone || !donationData.address ||
        !donationData.city || !donationData.pincode || !donationData.donationDate || 
        donationData.clothesDetails.length === 0 || !donationData.ngo) {
      ...
      <form onSubmit={handleDonationSubmit}>
        ...
        {clothesDetails.map((clothing, index) => (
          <div className="container-in" key={index}>
            ...
            <input className="inpt" type="text" name="quantity" placeholder="Quantity" value={clothing.quantity || ""} onChange={(e) => { handleClothesChange(index, e); setQuantities(e); }} required />
            <input className="inpt" type="text" name="damage" placeholder="Damage Percentage" value={clothing.damage} onChange={(e) => handleClothesChange(index, e)} required />
            <textarea name="description" rows="1" cols="50" placeholder="Enter description..." value={clothing.description} onChange={(e) => handleClothesChange(index, e)} required></textarea>
            <button className="btn" type="button" onClick={addRow}>ADD</button>
            <button className="btn" type="button" onClick={() => removeRow(index)}>X</button>
          </div>
        ))}
