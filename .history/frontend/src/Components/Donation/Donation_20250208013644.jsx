...
//         <input className="inpt" value={formData.name} onChange={changeHandler} type="text" name="name" placeholder="Your Name" required />
//         <input className="inpt" value={formData.email} onChange={changeHandler} type="email" name="email" placeholder="Your Email" required />
//         <input className="inpt" value={formData.phone} onChange={changeHandler} type="number" name="phone" placeholder="Enter Phone number" required />
...
//           <select className="inpt" value={formData.type} onChange={changeHandler} name="type" required>
...
const Donation = () => {
  ...
  const handleChange = (e, index = null) => {
    ...
    if (index !== null) {
      let updatedClothes = [...formData.clothesDetails];
  
      // Ensure the index exists before updating
      if (!updatedClothes[index]) {
        updatedClothes[index] = { type: "", quantity: "", damage: "" };
      }
  
      updatedClothes[index][name] = value;
      setFormData({ ...formData, clothesDetails: updatedClothes });
    } else {
  ...
  const addRow = (e) => {
    ...
    setFormData({
      ...formData,
      clothesDetails: [...formData.clothesDetails, { type: "", quantity: "", damage: "" }],
    });
  };
  ...
  const removeRow = (id) => {
    ...
    setFormData({
      ...formData,
      clothesDetails: formData.clothesDetails.filter((_, index) => index !== id),
    });
  };
  ...
  return (
    <div className="container">
      ...
      <form>
        ...
        <div className="container-in">
          <select className="inpt" name="type" value={formData.clothesDetails[0]?.type || ""} onChange={(e) => handleChange(e, 0)} required>
            <option value="">Select Type</option>
            <option value="T-shirt">T-shirt</option>
            <option value="Shirt">Shirt</option>
            <option value="Jeans">Jeans</option>
          ...
          <input className="inpt" type="text" name="quantity" value={formData.clothesDetails[0]?.quantity || ""} placeholder="Quantity" onChange={(e) => handleChange(e, 0)} required />
          <input className="inpt" type="text" name="damage" value={formData.clothesDetails[0]?.damage || ""} placeholder="Damage Percentage" onChange={(e) => handleChange(e, 0)} required />
          <button className="btns" onClick={addRow}>ADD</button>
        </div>

        {rows.map((row, index) => (
          <div className="container-in" key={row.id}>
            <select className="inpt" name="type" value={formData.clothesDetails[index]?.type || ""} onChange={(e) => handleChange(e, index)} required>
              <option value="">Select Type</option>
              <option value="T-shirt">T-shirt</option>
              <option value="Shirt">Shirt</option>
              <option value="Jeans">Jeans</option>
            ...
            <input className="inpt" type="text" name="quantity" value={formData.clothesDetails[index]?.quantity || ""} placeholder="Quantity" onChange={(e) => handleChange(e, index)} required />
            <input className="inpt" type="text" name="damage" value={formData.clothesDetails[index]?.damage || ""} placeholder="Damage Percentage" onChange={(e) => handleChange(e, index)} required />
            <button className="btn" onClick={() => removeRow(row.id)}>X</button>
          </div>
        ))}
