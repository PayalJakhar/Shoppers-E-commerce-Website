...
const Donation = () => {
  ...
  const setQuantities = (e, index) => {
    ...
    if (updatedClothes[index]) {
      updatedClothes[index].quantity = newQuantity;
      setClothesDetails(updatedClothes);
  
      const totalQuantity = updatedClothes.reduce((sum, item) => sum + (parseInt(item.quantity) || 0), 0);
      ...
      <form onSubmit={handleDonationSubmit}>
        ...
        {clothesDetails.map((clothing, index) => (
          <div className="container-in" key={index}>
            ...
            <input className="inpt" type="text" name="quantity" placeholder="Quantity" value={clothing.quantity} onChange={(e) => {handleClothesChange(index, e); setQuantities(clothing.quantity);}} required />
