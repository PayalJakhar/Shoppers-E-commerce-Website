...
const Donation = () => {
  ...
  const toggleNGO = (ngoUrl) => {
    setSelectedNGOs((prev) =>
      prev.includes(ngoUrl) ? prev.filter((url) => url !== ngoUrl) : [...prev, ngoUrl]
    );
  };
            ...
            <div className="modal-content">
              ...
              <div className="dropdown">
                <div className="dropdown-header" onClick={() => setIsHappen(!isHappen)}>
                  {selectedNGOs.length > 0 ? `${selectedNGOs.length} Selected` : "Choose NGOs"}
                </div>
                {isHappen && (
                  <div className="dropdown-menu">
                    {NGOs.map((ngo, index) => (
                      <label key={index} className="ngo-option">
                        <input
                          type="checkbox"
                          checked={selectedNGOs.includes(ngo.url)}
                          onChange={() => setSelectedNGOs(ngo.url)}
                        />
                        <span onClick={() => window.open(ngo.url, "_blank")}>{ngo.name}</span>
                      </label>
                    ))}
                  </div>
                )}
