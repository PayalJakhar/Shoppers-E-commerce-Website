...
const Donation = () => {
  ...
  const [NGOs, setNGOs] = useState([{name:"Any", url:"http//notfound"}]);
  ...
  useEffect(() => {
      const fetchNGOs = async () => {
          try {
              const response = await fetch('http://localhost:4000/allngos');
              if (!response.ok) throw new Error('Failed to fetch NGOs');

              const data = await response.json();
              if (data.success && Array.isArray(data.ngos)) {
                  setNGOs(data.ngos);  // ✅ Fix: Ensure NGOs is an array
              } else {
                  console.error("Invalid response format:", data);
                  setNGOs([]);  // ✅ Fix: Set empty array on error
              }
          } catch (error) {
              console.error("Error fetching NGOs:", error);
              setNGOs([]);  // ✅ Fix: Prevent `.map()` errors
          }
      };
  ...
  const handleDonationSubmit = async (e) => {
    e.preventDefault();
    if (!selectedNGOs) return alert("Please select an NGO before proceeding.");
    if (clothesDetails.length === 0) return alert("Please add at least one clothing item.");
    
    const formattedClothesDetails = clothesDetails.map(cloth => ({
      ...cloth,
      quantity: Number(cloth.quantity) || 0,
      damage: Number(cloth.damage) || 0,
    }));
    ...
    try {
      const response = await fetch("http://localhost:4000/donation", {
        ...
        body: JSON.stringify({ ...formData, clothesDetails: formattedClothesDetails, ngo: selectedNGOs.name }),
      });
  ...
  const handleDonation = async () => {
    ...
    try {
        ...
        const data = await response.json();
        if (data.success) {
            const updatedCountResponse = await fetch(`http://localhost:4000/donation/count/${userEmail}`);
            const updatedCountData = await updatedCountResponse.json();

            if (updatedCountData.success) {
                setDonationCount(updatedCountData.count);
            } else {
                console.error("Failed to fetch updated donation count:", updatedCountData.message);
            }
        } else {
                ...
                {isHappen && (
                  ...
                  {NGOs.map((ngo, index) => (
                    <label key={index} className="ngo-option">
                      ...
                      />
                      <a href={ngo.url} target="_blank" rel="noopener noreferrer">{ngo.name}</a>
                    </label>
                  ))}
                </div>
                )}
