...
const Donation = () => {
  ...
  const handleChange = (e, index = null) => {
    const { name, value } = e.target;
    if (index !== null) {
      let updatedClothes = [...clothesDetails];
      updatedClothes[index][name] = value;
      setClothesDetails(updatedClothes);
    } else {
      setFormData({ ...formData, [name]: value });
  ...
  const addRow = (e) => {
    ...
    setClothesDetails([...clothesDetails, { type: "", quantity: "", damage: "" }]);
  };

  const removeRow = (id) => {
    setRows(rows.filter((row) => row.id !== id));
    setClothesDetails(clothesDetails.filter((_, index) => index !== id));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Submitting:", formData, clothesDetails);
  };
  ...
  const handleDonationSubmit = async (e) => {
        ...
        const donationData = {
          ...formData, // Spread the form data
          clothesDetails,
          ngo: { name: ngo[selectedNGOs]?.name || "" },
          donationDate: document.getElementById("collection-date").value, // Get the selected date
        };
    ...
    <div className="container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="container-in">
          <select className="inpt" name="type" value={clothes.type}  onChange={(e) => handleClothesChange(index, e)} required>
            <option value="">Select Type</option>
          ...
          <input className="inpt" type="text" name="quantity" placeholder="Quantity"value={clothes.quantity}  onChange={(e) => handleClothesChange(index, e)} required />
          <input className="inpt" type="text" name="damage" placeholder="Damage Percentage" value={clothes.damgePercent}  onChange={(e) => handleClothesChange(index, e)} required />
          <textarea name="description" rows="1" cols="50" placeholder="Enter description..." value={clothes.description}  onChange={(e) => handleClothesChange(index, e)}></textarea>
