...
app.get('/donation/count/:email', async (req, res) => {
    try {
        ...
        const user = await Users.findOne({ email }, { Coins: 1 });

        if (!user) {
            return res.status(404).json({ success: false, message: "User not found" });
        }

        // Store total Coins in donationCount
        const donationCount = user.Coins;
...
// ✅ Route to verify paymentapp.post('/api/payment/verify', async (req, res) => {
    try {
