...
app.post('/api/payment/verify', async (req, res) => {
    try {
        ...
        if (user.Coins >= 500) {
            user.Coins -= 500;
            await user.save(); // Ensure database update
        } else {
            return res.status(400).json({ success: false, message: "Not enough coins!" });
        }

        // Fetch updated coins count
        const updatedUser = await Users.findOne({ email: emailId }, { Coins: 1 });

        return res.status(200).json({
            success: true,
            message: "Payment Verified Successfully",
            updatedCoins: updatedUser.Coins // Return updated coins
        });
