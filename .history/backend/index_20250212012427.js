...
const Users = mongoose.model('Users', {
    ...
    donationCount: { type: Number, default: 0 } // Ensure this field exists
});
...
app.get('/donation/count/:email', async (req, res) => {
    try {
        ...
        const user = await Users.findOne({ email }, { donationCount: 1 });

        if (!user) {
            return res.status(404).json({ success: false, message: "User not found" });
        }

        return res.status(200).json({ success: true, count: user.donationCount });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ success: false, error: err.message });
    }
...
app.post('/api/payment/verify', async (req, res) => {
    try {
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, emailId, token } = req.body;

        if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !emailId || !token) {
        ...
        await Payment.create({ razorpay_order_id, razorpay_payment_id, razorpay_signature, emailId });

        const updatedUser = await Users.findOneAndUpdate(
            { email: emailId },
            { $inc: { Coins: -500, donationCount: -500 } },
            { new: true }
        );
        ...
        return res.status(200).json({ 
            ...
            updatedDonationCount: updatedUser.donationCount 
        });
