...
//Acts as a backend framework to handle HTTP requests and responses(express).
...
}); //file.fieldname → The form field name of the uploaded file (e.g., "avatar" if the file is uploaded via <input name="avatar">).
    //Date.now() → Generates a unique timestamp to avoid filename conflicts.
    //path.extname(file.originalname) → Extracts the original file extension (.jpg, .png, etc.).
    //req → The HTTP request object.
    //file → The uploaded file's metadata (contains originalname, fieldname, etc.).
...
app.use('/images', express.static('upload/images'));//we here make it accessible using /images

app.post("/upload", upload.single('product'), (req, res) => {
    res.json({
        success: 1,
        image_url: `http://localhost:${port}/images/${req.file.filename}`
    }); // so our users will get image in json format and single product image to be uploaded
});
...
const ClothesSchema = new mongoose.Schema({
    type: {
      type: String,
      required: true,
      enum: ['T-shirt', 'Shirt', 'Jeans', 'Trouser'],//enum is for the type value can be from these only
    },
    quantity: {
      type: Number,
      required: true,
  ...
  app.post("/donation", async (req, res) => {
    try {
        ...
        user1.coins += 100;
        user1.donationCount += 1; // Corrected: Increment by 1, not 500
        await user1.save();
        await donation.save();
...
app.post('/addtocart', fetchUser, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Item added to cart', cartData: userData.cartData });
    } catch (error) {
...
app.post("/api/payment/verify", async (req, res) => {
    try {
        ...
        // ✅ Reduce 500 coins from the user's balance
        let cart = {};
        for (let i = 0; i < 300; i++) {
            cart[i] = 0;
        }
        if (user.coins >= 500) {
            ...
            user.cartData=cart;
            await user.save();
            return res.status(200).json({ success: true, message: "Payment Verified & Coins Deducted Successfully" });
        } else {
            user.cartData=cart;
            await user.save();
