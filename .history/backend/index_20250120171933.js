...
app.use(express.json()); // Automatically parse JSON responses
app.use(cors()); // Allow React project to connect to port 4000
...
app.get('/newcollections', async (req, res) => {
    let products = await Product.find({});
    let newcollection = products.slice(1).slice(-8); // Take the last eight elements
...
app.post('/addtocart', fetchUser, async (req, res) => {
    ...
    await Users.findOneAndUpdate({_id:req.user.id},{cartData:userData.cartD})
});
